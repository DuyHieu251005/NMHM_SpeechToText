...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Wav2Vec2}: \texttt{nguyenvulebinh/wav2vec2-base-vietnamese-250h}
        \item \textbf{PhoWhisper}: \texttt{vinai/PhoWhisper-small}
        \item \textbf{OpenAI Whisper}: \texttt{openai/whisper-small}
    \end{itemize}
    
    \item \textbf{Audio Processing:}
    \begin{itemize}
        \item Librosa để load và resample audio về 16kHz
        \item Chuyển đổi định dạng (WAV, MP3, M4A, FLAC)
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Load audio với librosa (resample về 16kHz)
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.9\textwidth]{img/ui_main.png}
...
\begin{itemize}
    ...
    \item \textbf{Model Selector}: Dropdown chọn mô hình AI (Wav2Vec2, PhoWhisper, Whisper)
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item PhoWhisper: Tối ưu cho tiếng Việt (mặc định)
        \item OpenAI Whisper: Mô hình đa ngôn ngữ mạnh mẽ
        \item Wav2Vec2: Mô hình nhẹ, nhanh
    \end{itemize}
...
\hline
Librosa & Xử lý audio (load, resample) \\
\hline
